        ...
        <div id="mainSideMenu" className={`sidebar px-4 py-4 py-md-5 me-0 ${isSidebarMini ? "sidebar-mini" : ""}`}>
            <div className="d-flex flex-column h-100">
                ...
                <ul className="menu-list flex-grow-1 mt-3">
                    {
                        menuData.map((d, i) => {
                            const lowercaseName = d.permissions;
                            if (permissions[lowercaseName] !== undefined && !permissions[lowercaseName]) {
                                console.log("permissions deyed for " + lowercaseName)
                                return null
                            }
                            if (d.isToggled) {
                                    ...
                                    d.children.length > 0 ?
                                        <ul className="sub-menu collapse has-children" id={"menu-Pages" + i}>
                                            {d.children.map((data, ind) => {
                                                if (d.children.length > 0) {
                                                    const lowercaseName = data.permissions;
                                                    if (permissions[lowercaseName] !== undefined && !permissions[lowercaseName]) {
                                                        console.log("permissions deyed for " + lowercaseName)
                                                        return null
                                                    }
